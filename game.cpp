...
static void build_level_geometry(int lvl) {
  ...
  // Las cuentas reales y el llenado de los inst_buf de los aliens se hacen
  // todos los frames dentro de `frame_cb`. Por consiguiente, NO debemos llamar
  // `sg_update_buffer` aquí, ya que causaría un panico en Sokol si esta funcion
  // se invoca durante el transcurso del juego (error: double buffer update).
